...
const paginationAdmin = async (page: number, count: number) => {
  const data = await prisma.pG.findMany({
    where: {
      deletedAt: {
        isSet: false,
      },
  ...
  const dataCount = await prisma.pG.count({
    where: {
      deletedAt: {
        isSet: false,
      },
  ...
  const hasNext = await prisma.pG.findMany({
    take: 1,
    skip: count * (page + 1 - 1),
    where: {
      deletedAt: {
...
const paginationUser = async (page: number, count: number, user: any) => {
  ...
  for (const { batchId } of getCoursePurchased) {
    ...
    dataCount = await prisma.pG.count({
      where: {
        courseFolder: {
          course: {
            batchCourse: {
    ...
    hasNext = await prisma.pG.findMany({
      take: 1,
      skip: count * (page + 1 - 1),
      where: {
        courseFolder: {
